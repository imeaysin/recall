...
type CapButtonSize = "xs" | "sm" | "md" | "lg" | "icon"

export interface ButtonProps extends Omit<
  React.ComponentProps<typeof SharedButton>,
  "variant" | "size" | "render"
> {
  variant?: CapButtonVariant
  size?: CapButtonSize
  href?: string
  target?: string
  icon?: ReactNode
...
const variantClasses: Record<CapButtonVariant, string> = {
  primary: "bg-foreground text-background hover:bg-foreground/85",
  blue: "bg-primary text-primary-foreground hover:bg-primary/85",
  destructive:
    "bg-destructive text-destructive-foreground hover:bg-destructive/85",
  outline: "border-border bg-background text-foreground hover:bg-muted",
  white: "border-border bg-card text-card-foreground hover:bg-muted",
  ghost: "bg-transparent text-foreground hover:bg-muted",
  gray: "border-border bg-muted text-foreground hover:bg-muted/75",
  dark: "bg-foreground text-background hover:bg-foreground/85",
  darkgradient:
    "border-transparent bg-linear-to-t from-foreground to-muted-foreground text-background hover:brightness-110",
  radialblue:
    "border-transparent bg-[radial-gradient(90%_100%_at_15%_12%,var(--color-accent)_0%,var(--color-primary)_100%)] text-primary-foreground hover:opacity-85",
  transparent:
...
export function Button({
  ...
  const classes = cn(
    buttonVariants({ variant: "outline", size: sizeMap[size] }),
  ...
  const content = (
    <>
      ...
      {children}
      {kbd ? (
        <kbd className="rounded-full border border-current/20 px-1.5 text-xs">
          {kbd}
        </kbd>
      ) : null}
    </>
