...
Theo v0.5 is here, and it is easily the biggest desktop release we have shipped so far.

We did not just want this release to be a pile of features. We wanted the everyday parts of Theo to feel better: recording, editing, sharing, and deciding where your videos live.
...
This is not a tiny version bump. It is a serious upgrade to how Theo works, and to where we are taking the product next.

Some of it is obvious the moment you open the app. Some of it is less glamorous, but just as important.

Theo is supposed to disappear while you work. You should be able to hit record, explain the thing, clean it up, and share it without wondering whether the audio drifted, the export stalled, the camera preview froze, or the recording can be recovered if something goes wrong.
...
If you record in Instant Mode, or create a shareable link from Studio Mode, Theo can upload and serve that video from your connected Google Drive.

This is a big step toward giving you more control over where your recordings live. We have always wanted Theo to feel local-first and user-owned, and storage choice is part of that.

## Merge recordings and import local videos

Studio Mode is now more flexible.

You can bring previous Studio recordings into the editor and merge them with your current project. You can also import local videos, add media from the tray or the main window, open things from recording settings, or just drag and drop a file into Theo.
...
It sounds small, but it changes how Theo feels. Recordings are no longer isolated sessions. They can become projects you build on.
...
You can choose the blur strength from camera controls or from the editor sidebar. Theo will use GPU acceleration where it can, with Core ML on macOS and DirectML on Windows, and fall back to CPU when needed.
...
If you use Theo with an organization, the editor can now pull in your brand colors and logos.
...
Compatibility mode lowers bitrate and capture pressure for older or lower-power machines. Balanced is the default path for most people. Ultra is there when you want the highest quality Theo can push.
...
We tightened a lot of the recording pipeline. Theo now does better disk-space checks before recording, tracks recording health while capture is running, and can recover more sessions after a crash or forced quit.
...
Theo now reconciles late or overlapping microphone audio against the capture clock instead of accidentally layering in duplicate silence. It also handles unexpected gaps, cancelled capture pipelines, Windows system audio, microphone resampling, decoded sources, and surround downmixing more carefully.
...
On macOS, Theo excludes more Theo-owned windows from recordings by default, including Tauri webviews and overlays. We also fixed duplicate Dock icons, app switcher persistence, panel focus issues, and the hide Dock icon setting.
...
You can download Theo v0.5 at [Theo.so/download](https://theo.example/download).

This release has a lot in it, and not all of it is visible on the surface. That is intentional. We want Theo to feel faster, calmer, and more dependable every time you record.
...
You can reach us at [support@theo.example](mailto:support@theo.example), or use our [direct Loom importer](/dashboard/import/loom) with a CSV to migrate all of your workspace at once.

As always, thank you to everyone using Theo, testing builds, sending crash reports, opening issues, and telling us where the app still feels rough.
